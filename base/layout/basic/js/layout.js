...
function fixedHeader() {
  ...
  // var header_height = document.getElementById("header").offsetHeight+'px';
...
jQuery(document).ready(function () {
  ...
  /* 최상단배너 하루동안 닫기 - 서정환 */
  jQuery(".main_top_banner .top_banner_box_inner .top_banner_close .icon").bind(
    "click",
    function () {
  ...
  var main_top_banner_diplay =
    jQuery(".main_top_banner").attr("data-ez-display");
  ...
  /* 상단 고객센터 - 서정환 */
  jQuery("#header .inner .toparea .toparea_state .toparea_state_board").click(
    function () {
  ...
  jQuery("#footer .inner .bt_escrow").each(function () {
    ...
    if (!bt_escrow == "") {
      var bt_escrow_name = jQuery(
        "a img[data-ez-escrow-id=" + bt_escrow + "]",
        this,
      ).addClass("on");
...
function ifmore() {
  ...
  jQuery(".ec-base-product .prdList > li").each(function () {
    ...
    /* 상품진열 관심상품 사용안할시 숨김 */
    if (
      jQuery(".icon__box .wish > .ec-product-listwishicon", this).length == 1
    ) {
    } else {
